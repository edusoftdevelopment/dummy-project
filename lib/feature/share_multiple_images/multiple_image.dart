...
class ReceiptLedgerPage extends StatefulWidget {
  const ReceiptLedgerPage({super.key});

  @override
  State<ReceiptLedgerPage> createState() => _ReceiptLedgerPageState();
}

class _ReceiptLedgerPageState extends State<ReceiptLedgerPage> {
  ...
  Future<void> _captureAndShare() async {
    try {
      ...
      final boundary =
          _ledgerKey.currentContext?.findRenderObject()
              as RenderRepaintBoundary?;
      if (boundary == null) throw 'Ledger not rendered yet';

      final image = await boundary.toImage(pixelRatio: 3);
      final byteData = await image.toByteData(format: ui.ImageByteFormat.png);
      if (byteData == null) throw 'Unable to convert to bytes';

      final pngBytes = byteData.buffer.asUint8List();
      final tempDir = await getTemporaryDirectory();

      final file = await File(
        '${tempDir.path}/ledger_${DateTime.now().millisecondsSinceEpoch}.png',
      ).create();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(_isSaving ? 'Sharing...' : 'Share'),
      ),
      body: SafeArea(
        child: Center(
